...
def volume_graphs(request):
    ...
    hourly_results = TopHeadlines.objects.filter(published_at__gte=datetime.now()-timedelta(days=2)).annotate(hour=TruncHour('published_at')).values('hour').annotate(daily_results=Count('published_at')).order_by('hour')[:24]
